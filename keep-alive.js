      ...
      if (response.statusCode === 200) {
        try {
          ...
          if (health.status === "healthy") {
            console.log(
              `✅ Backend is healthy! Server timestamp: ${health.timestamp}`
            );
          } else {
            console.log(
              `⚠️  Backend responded but status is: ${health.status}`
            );
          }
        } catch (parseError) {
          console.log(
            `✅ Backend responded with 200 (JSON parse error: ${parseError.message})`
          );
        }
        return true;
      } else if (response.statusCode === 503) {
        console.log(
          `🔄 Backend is spinning up (503) - attempt ${attempt}/${CONFIG.MAX_RETRIES}`
        );
...
async function startKeepAlive() {
  ...
  console.log(
    `🔄 Keep-alive service is running. Next ping in ${
